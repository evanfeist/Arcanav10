...
(function(){
  ...
  window.sendTurn = function(action){
    if (G.net.role==='host' && (!G.net.connected || !G.net.dc)) {
      ...
      window.arcana.render();                   // <-- host render offline
      return;
    }
  ...
  function hookDC(dc){
    ...
    dc.onopen = ()=>{
      ...
      // (removed baseline SYNC to avoid double-RX on guest)
    };
  ...
  function handlePacket(p){
    ...
    // (Ace drawer is handled on guest by a separate handler if you add it here)
  }
  ...
  async function hostResolveAndBroadcast(action){
    ...
    if(action.type==='ACTION_7' || action.type==='ACTION_9'){
      ...
      if(window.arcana.hasAce(target)){
        ...
        if(useAce){
          ...
          window.arcana.render();                                // <-- host render after Ace
    ...
    window.arcana.applyTurn(action);
    window.arcana.render();                                      // <-- host render after action
    dcSend({ kind:'APPLY', state: window.arcana.serialize() });
  }
  ...
  btnStart.addEventListener('click', ()=>{
    ...
    window.arcana.render();                     // <-- render immediately on host
  ...
  window.pushLog = (s)=>{}; // optional hook
})();