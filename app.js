...
(() => {
  ...
  const aceModal=$('aceModal'), aceText=$('aceText'), aceNo=$('aceNo'), aceYes=$('aceYes');
  ...
  function wsConnect(url){
    ...
    G.net.ws.onmessage = async (e)=>{
      ...
      if(msg.type==='host-ok'){ setRole('host'); G.net.hostId=msg.hostId; $('hostId').textContent=msg.hostId; setStatus('Host ready. Share ID.'); await hostPeer(); return; }
  ...
  function handlePacket(p){
    ...
    if(p.kind==='SYNC'){
      window.arcana.hydrate(p.state); window.arcana.render(); log('RX SYNC ✓');
      return;
    }

    if(p.kind==='TURN'){
      // Host already applied and synced? In this build Host applies then SYNCs, so TURN is only from guest to host.
    ...
    if(p.kind==='ACE_OFFER'){
      // Guest sees modal
      aceText.textContent = p.context.for==='A7' ? 'Opponent played 7. Use Ace to attempt a block?' : 'Opponent played 9. Use Ace to attempt a block?';
      aceModal.classList.add('open');
      // store context
      aceModal.dataset.ctx = p.context.for;
      return;
    }
  }
  ...
  async function hostResolve(action){
    ...
    if(action.type==='ACTION_7' || action.type==='ACTION_9'){
      ...
      if(window.arcana.hasAce(target)){
        ...
        const use = await waitForAceReply(); // true/false
        if(use){
          // roll d6: 1–3 fail (action succeeds), 4–6 block + counter 1
          const roll = 1+Math.floor(Math.random()*6);
          log(`Ace roll = ${roll}`);
          if(roll>=4){
            // apply counter
            window.arcana.applyTurn({ type:'ACE_REACT', from:target, payload:{ used:true, for:action.type } });
            // mark original as blocked (no effect)
            action.payload = { ...(action.payload||{}), blocked:true };
            // still discard the action card (host will do in applyTurn of that action)
            // For clarity: we treat block as: apply ACE_REACT (counter) + consume action card via applyTurn(blocked)
          }
        }
      }
    }

    // Apply action (host authority)
  ...
  function waitForAceReply(){
    return new Promise((resolve)=>{
      // Temporarily intercept TURN packets carrying ACE_REPLY
  ...
  aceNo.addEventListener('click', ()=>{ aceModal.classList.remove('open'); dcSend({kind:'TURN', action:{ type:'ACE_REPLY', payload:{ use:false } }}); });
  aceYes.addEventListener('click', ()=>{ aceModal.classList.remove('open'); dcSend({kind:'TURN', action:{ type:'ACE_REPLY', payload:{ use:true } }}); });
