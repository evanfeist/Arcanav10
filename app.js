// app.js — P2P transport (signaling + WebRTC) with Ace handshake + guest log
(function(){
  ...
  function handlePacket(p){
    ...
    // Host -> Guest: show Ace modal to decide block on 7/9
    if(p.kind==='ACE_OFFER'){
  ...
  function hostResolveAndBroadcast(action){
    if(action && (action.type==='ACTION_7' || action.type==='ACTION_9')){
      ...
      if(window.arcana.hasAce(target)){
        ...
        // Wait for guest reply exactly once
        ...
        return; // pause here until ACE_REPLY arrives
      }
