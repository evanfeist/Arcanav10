...
(function(){
  ...
  const G = {
    ...
    turn: 0,
    started: false,
    extraChain: 0,           // for clubs bonus (max chain 2 turns total)
    localSide: 'P1',         // 'P1' if host, 'P2' if guest (set by app.js)
    selected: new Set(),     // selected card ids (local player's hand)
  };
  ...
  function backNode(){
    ...
    d.style.background='linear-gradient(180deg,#151e2a,#0f1620)'; d.style.border='1px solid #2a3a4e';
    return d;
  }
  ...
  function log(s){
    ...
    if(window.pushLog) window.pushLog(s); // app.js mirrors to guest when host
  }
  ...
  function validateConstruct(cards, pidx){
    ...
    if (jokers.length === 0 && nonj.every(c => c.rank === nonj[0].rank) && cards.length >= 3) {
      ...
      if (n === 3) pts = 2 * r + 2;
        ...
        pts = (suitsSet.size === 4) ? (4 * r) : (3 * r); // one-per-suit special
      } else if (n >= 5) {
        pts = 3 * r + 2;
      }
    ...
    if (nonj.length >= 2 && nonj.every(c => c.rank === nonj[0].rank) && cards.length >= 3 && jokers.length > 0) {
      ...
      res.valid=true; res.kind='SET'; res.points=pts;
      // suit bonus allowed for n >= 6 even with jokers
    ...
    // ---- Straight (Run): same suit, consecutive; jokers can fill gaps ----
    if(sameSuit && cards.length>=3){
      ...
      if(gaps<=jokers.length){
        ...
        res.points = sumVals + len; // +1 per card (incl. jokers)
  ...
  // ---------- Attempt Construct from current selection ----------
    ...
    switch(action.type){
      ...
      case 'ACE_REACT': {
        if(action.payload?.used){
          ...
          G.players[actor].score -= give; G.players[target].score += give;
  ...
  function openGiveDrawer(nineCard){
    // list all my cards except the 9
    const pidx = me();
    giveList.innerHTML='';
  ...
  btnAction.addEventListener('click', ()=>{
    ...
    // 9: if only the 9 is selected, open the give drawer; if two selected (9 + another), send immediately
    if(sel.length===1 && sel[0].rank==='9'){
      openGiveDrawer(sel[0]); return;
    }
