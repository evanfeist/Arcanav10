...
\f0\fs24 \cf0 // game.js \'97 Arcana core rules + UI wiring (Stage 1\'963) with Joker Cost drawer\
\
(function()\{\
  const el = (id)=>document.getElementById(id);\
  const logEl = el('log');\
\
  const SUITS = ['\uc0\u9824 ','\u9829 ','\u9830 ','\u9827 '];\
...
\
  ...
  function buildDeck()\{\
    ...
    for(let d=0; d<2; d++)\{\
      ...
      cards.push(\{id:id++, suit:'\uc0\u9733 ', rank:'Joker', joker:true\});\
      cards.push(\{id:id++, suit:'\uc0\u9734 ', rank:'Joker', joker:true\});\
    \}\
...
\
  ...
  function render()\{\
    ...
    el('turnTag').textContent = G.started ? (G.turn===0?'P1':'P2') : '\'97';\
\
...
\
  function log(s)\{\
    ...
    if(window.pushLog) window.pushLog(s);\
  \}\
\
  // ---------- Helpers ----------\
...
\
    ...
    if(nonj.length===0)\{\
      ...
      res.reason='Need 2\'964 Jokers for Joker-only'; return res;\
    \}\
\
...
\
  function applySuitBonusPreview(res, allowWithJokers)\{\
    ...
    if(res.suit==='\uc0\u9829 ')\{ res.points += 2; res.suitBonus='HEARTS'; \}\
    if(res.suit==='\uc0\u9827 ')\{ res.suitBonus='CLUBS'; \}\
    if(res.suit==='\uc0\u9830 ')\{ res.suitBonus='DIAMONDS'; \}\
    if(res.suit==='\uc0\u9824 ')\{ res.suitBonus='SPADES'; \}\
  \}\
\
  // ---------- Attempt Construct ----------\
...
\
        ...
        if(check.suitBonus==='DIAMONDS')\{\
          ...
          log(`\uc0\u9830  bonus d6=$\{roll\} \u8594  stole $\{give\} point(s).`);\
        \}\
        if(check.suitBonus==='SPADES')\{\
          ...
          \}\
          log(`\uc0\u9824  bonus: stole up to 2 random card(s).`);\
        \}\
...
\
      case 'DISCARD': \{\
        ...
        log((actor===0?'P1':'P2')+` discarded $\{c? (c.rank+c.suit) : '\'97'\}.`);\
        endTurn(); return;\
      \}\
    \}\
  \}\
